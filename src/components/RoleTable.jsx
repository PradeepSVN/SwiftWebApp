...
const columns = [
  { id: 'role_ID', label: 'Role Id',numeric: false, disablePadding: true,orderBy:'asc', minWidth: 150 },
  {id: 'role_Name', label: 'Role Name',numeric: false, disablePadding: true,orderBy:'asc', minWidth: 180 },
            ...
            {tableData.rows && tableData.rows.length >0? stableSort(tableData.rows, getComparator(order, orderBy)).map((row,index) => 
              {
                return (<>            
                  <TableRow style={{height:'10px'}}></TableRow>
                   <TableRow style={{height: '60px !important'}} role="checkbox" tabIndex={-1} key={row.user_ID} className='table-row1'
                    // onClick={(event) => handleRowChange(event, row)} 
                     >
                    {columns.map((column) => {
